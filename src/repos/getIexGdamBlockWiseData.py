...
def getIexGdamBlockWiseData(appDbConnStr: str, col_attributes: str, startDt: dt.datetime, endDt: dt.datetime) -> List[IMetricsDataRecord]:
    targetColumns = ['TRUNC(TIME_STAMP)', 'COL_ATTRIBUTES', 'DATA_VALUE']

    metricsFetchSql = """

            select {0} from 
            mo_warehouse.iex_gdam where time_stamp >= :1
            and time_stamp < :2
    ...
    try:
        ...
        dbRows = []
        print('Error while fetching iex Gdam data between dates')
        print(err)
    finally:
